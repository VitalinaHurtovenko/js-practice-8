...
function removeLastElement(arr) {
  ...
  arr.length === 0 ? undefined : arr.pop();
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    ...
    let secondIndex = arr.lastIndexOf(element);
    let newArr = [];
 ...
 */
function customFill(arr, value, start = -3, end = arr.length) {
  // Перевірка вхідних параметрів
  // якщо arr не масив повертаємо false
  if (
    !Array.isArray(arr) ||
...
function customShift(arr) {
  ...
  if (!Array.isArray(arr) && arr.length === 0) {
    ...
    console.log({ shiftedElement, arr });
  }
...
function customUnshift(arr, ...elements) {
  ...
  console.log({ initialLength, newLength, arr });
...
function customAt(arr, index) {
  ...
  if (typeof element === "string") {
    return `${element}: Елемент є рядком`;
  } else if (typeof element === "number") {
    return `${element}: Елемент є числом`;
  } else if (typeof element === "object") {
    return `${element}: Елемент є обєктом`;
  }
}
...
function customIncludes(arr, element) {
  ...
  let numOfElem = arr.filter((item) => item === element);

  return `${numOfElem.length}: ${arr.includes(element)}`;
